    ...
    <div class="row">
        <div class="col-xl-6 mx-auto">
            <div class="card-box mt-3">
                ...
                <form action="{{route('superAdmin.userProfileEdit')}}" method="POST" enctype="multipart/form-data">
                    @csrf

                    <div class="form-group">
                        <label for="email">{{__('portal.Email')}}</label>
                        <input type="email" parsley-trigger="change" id="email" placeholder="{{__('portal.Enter email')}}" class="form-control" value="{{auth()->user()->email}}" disabled>
                    </div>
