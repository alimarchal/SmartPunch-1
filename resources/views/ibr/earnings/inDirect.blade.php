...
@section('body')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box">
                <h4 style="text-align: center;">{{__('portal.List of indirect earnings')}}</h4>
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap">
                    ...
                    <tbody>
                    @foreach($inDirectEarnings as $indirectEarning)
                        <tr>
                            <td>{{$loop->iteration}}</td>
                            <td>{{$indirectEarning->directCommission->transaction->business->company_name}}</td>
                            <td>{{$indirectEarning->directCommission->ibr_no}}</td>
                            <td>{{$indirectEarning->amount}}</td>
                            <td>{{\Carbon\Carbon::parse($indirectEarning->created_at)->format('d/M/Y')}}</td>
                            <td>
                                @if($indirectEarning->status == 0)
                                    <span style="color: red">{{__('portal.Unpaid')}}</span>
                                @elseif($indirectEarning->status == 1)
